...
function numberOfVowel(string) {
  let vowelArr = {'A': true, 'E':true, 'I': true, 'O': true, 'U': true, 'a':true, 'e':true, 'i':true, 'o':true, 'u':true};
  let count = 0;
...
function perfectNumber(number) {
  return [...Array(number).keys()].slice(1).filter(item=>number % item === 0).reduce((acc,cur)=>acc+cur) === number;
}
